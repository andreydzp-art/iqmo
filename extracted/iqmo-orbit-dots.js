...
(function (global) {
	...
	function build(daysCount, opts) {
		...
		for (var i = 0; i < n; i++) {
			...
			// Индивидуальные параметры точки — чтобы они визуально не
			// сливались в один синхронно крутящийся обруч. Все формулы
			// детерминированы по индексу: при перерисовке (patchHero на
			// iqmo-sync) точки не «прыгают».
			//   --dur  : длительность полного оборота. 5 уровней скорости
			//            (22..32s), точки постепенно расходятся.
			//   --dir  : направление вращения. Чёт — по часовой, нечёт —
			//            против часовой → ощущение живой системы.
