...
const FAQ = () => {
  ...
  const items = [
    ...
    {
      q: "Чем формат IQMO отличается от обычной подготовки?",
      a: "Это интерактивный тренажёр с прогрессом по каждой теме. Ребёнок занимается каждый день по 10–15 минут, видит ошибки сразу и возвращается к слабым темам автоматически. Платформа адаптируется под ученика и не зависит от расписания учителя — практика возможна в любое удобное время.",
    },
    {
    ...
    {
      q: "Что если ребёнку не понравится формат?",
      a: "Регистрация без оплаты — вы и ребёнок можете спокойно открыть платформу, посмотреть карту тем, пройти первые задания и решить, подходит ли формат. Прогресс сохраняется в вашем профиле и не теряется между сессиями.",
    },
    {
