#!/usr/bin/env node
/**
 * Импорт вариантов 1–10 из PDF «ОГЭ химия 2026» → site variants 30–39.
...
const SOURCE = process.argv[2] || 'C:/Users/user/Downloads/oge_chemistry (1).js';
...
const blocks = [];
for (let pdfV = 1; pdfV <= 10; pdfV++) {
	const siteV = pdfV + SITE_VARIANT_OFFSET;
	const qs = byPdfVariant.get(pdfV).sort((a, b) => a.type - b.type);
	blocks.push(`\t\t\t\t// ============================================================`);
	blocks.push(`\t\t\t\t// ВАРИАНТ ${siteV} · ОГЭ химия 2026, PDF вариант ${pdfV} (${siteV * 100 + 1}–${siteV * 100 + qs.length})`);
	blocks.push(`\t\t\t\t// Источник: oge_chemistry (1).js`);
	blocks.push(`\t\t\t\t// ============================================================`);
	for (const q of qs) {
...
const marker = '// ВАРИАНТ 30 · ОГЭ химия 2026';
if (bank.includes(marker)) {
	bank = bank.replace(/,\n\t\t\t\t\/\/ =+\n\t\t\t\t\/\/ ВАРИАНТ 30 · ОГЭ химия 2026[\s\S]*$/, '\n];\n');
	writeFileSync(bankPath, bank);
	console.log('Removed previous import block for re-import.');
}
if (bank.includes(marker)) {
...
for (let pdfV = 1; pdfV <= 10; pdfV++) {
	const siteV = pdfV + SITE_VARIANT_OFFSET;
	const qids = [];
	for (let t = 1; t <= 23; t++) qids.push(siteV * 100 + t);
	newVariants.push(`\t// Вариант ${siteV} — ОГЭ химия 2026, PDF вариант ${pdfV}\n\tvariants.push({\n\t\tid: ${siteV},\n\t\ttitle: 'Вариант ${siteV}',\n\t\tqids: [${qids.join(',')}],\n\t\tstatus: 'ready'\n\t});`);
}

if (varSrc.includes('// Вариант 30 — ОГЭ химия 2026')) {
	varSrc = varSrc.replace(
		/\t\/\/ Вариант 30 — ОГЭ химия 2026[\s\S]*?\tfor \(let i = 40/,
		'\tfor (let i = 30'
	);
	writeFileSync(varPath, varSrc);
	console.log('Reset variants 30–39 stubs for re-import.');
}
...
if (varSrc.includes('// Вариант 30 — ОГЭ химия 2026')) {
	...
	writeFileSync(varPath, varSrc);
	console.log('Updated chemistry-variants.js (variants 30–39 ready)');
}
