#!/usr/bin/env node
/**
 * Импорт вариантов 11–20 из PDF «ОГЭ химия 2026» → site variants 40–49.
 * Источник: oge_chemistry_11-20.js
 */
...
const SOURCE = process.argv[2] || 'C:/Users/user/Downloads/oge_chemistry_11-20.js';
const SITE_VARIANT_OFFSET = 29; // PDF variant N → site variant N + 29
const PDF_FIRST = 11;
const PDF_LAST = 20;
const SITE_FIRST = PDF_FIRST + SITE_VARIANT_OFFSET; // 40
const SITE_LAST = PDF_LAST + SITE_VARIANT_OFFSET; // 49
const SOURCE_LABEL = 'oge_chemistry_11-20.js';
...
function convertQuestion(q, siteVariant) {
	...
	if ([1, 5, 6, 8, 11, 13].includes(q.type)) {
		...
		if (q.type === 20) {
			const eq = q.question.match(/схема которой[:\s]+(.+?)\.\s*Запишите/is) || q.question.match(/схеме реакции[:\s]+(.+?)\.\s*Запишите/is);
			if (eq) {
...
const endMarker = `// ВАРИАНТ ${SITE_LAST + 1} · ОГЭ химия 2026`;
if (bank.includes(marker)) {
	const re = new RegExp(
		`,\\n\\t\\t\\t\\t\\/\\/ =+\\n\\t\\t\\t\\t\\/\\/ ВАРИАНТ ${SITE_FIRST} · ОГЭ химия 2026[\\s\\S]*?(?=\\n\\t\\t\\t\\t\\/\\/ =+\\n\\t\\t\\t\\t\\/\\/ ВАРИАНТ ${SITE_LAST + 1} · ОГЭ химия 2026)`
	);
	if (bank.match(re)) {
		bank = bank.replace(re, '');
...
if (bank.includes(marker)) {
	...
	bank = bank.replace(
		/(\n\t\t\t\t\/\/ =+\n\t\t\t\t\/\/ ВАРИАНТ 50 · ОГЭ химия 2026)/,
		',\n' + blocks.join('\n') + '\n$1'
	);
	writeFileSync(bankPath, bank);
	console.log(`Inserted ${questions.length} questions into chemistry-bank.js (before variant 50)`);
}
...
if (varSrc.includes(varMarker)) {
	varSrc = varSrc.replace(
		new RegExp(`\\tfor \\(let i = 40; i <= 49; i\\+\\+\\) \\{[\\s\\S]*?\\}\\n\\t(?:\\/\\/ Вариант ${SITE_FIRST}[\\s\\S]*?)+`),
		''
	);
	writeFileSync(varPath, varSrc);
	console.log(`Reset variants ${SITE_FIRST}–${SITE_LAST} for re-import.`);
}
...
if (varSrc.includes(varMarker)) {
	...
	varSrc = varSrc.replace(
		...
		readyBlock || `\tfor (let i = 40; i <= 49; i++) {\n\t\tvariants.push({ id: i, title: 'Вариант ' + i, qids: [], status: 'coming' });\n\t}`
	);
	writeFileSync(varPath, varSrc);
	console.log(`Updated chemistry-variants.js (variants ${SITE_FIRST}–${SITE_LAST})`);
}
