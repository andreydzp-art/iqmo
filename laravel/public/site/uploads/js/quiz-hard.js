(function () {
	var container = document.querySelector('.quiz-hard .quiz__container');
	if (!container) return;
	...
	function topicHighlight() {
		var topic = items[idx].getAttribute('data-topic');
		document.querySelectorAll('.topic-categories__item').forEach(function (t) {
			t.classList.toggle('is-active', t.getAttribute('data-index') === topic);
		});
	}
