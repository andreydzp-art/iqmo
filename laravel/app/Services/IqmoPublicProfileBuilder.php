...
final class IqmoPublicProfileBuilder
{
    ...
    private static function buildPayload(int $userId, string $email, int $createdAtMs, array $keys): array
    {
        ...
        $mathV = self::summarizeVariants($keys, 'iqmo-math-v-', range(1, 24));
        ...
        // Математика: 24 готовых варианта. $mathV['total'] = 24 благодаря
        // range(1, 24), переданному в summarizeVariants выше. Когда добавим
        ...
        $subjects = [
            ...
            self::subjectCard('mathematics', 'Математика', 'ОГЭ · 24 варианта · 25 заданий', $mathPct, $mathV, '/full-test-mathematics/', $level['current']),
        ];
    ...
    private static function resolveDisplayName(array $keys, string $email): string
    {
        ...
        // Приватность (audit): публичный профиль перечислим по IQ-id, а
        // аудитория — несовершеннолетние. Восстанавливать имя из e-mail здесь
        // НЕЛЬЗЯ — перебор id раскрыл бы ПДн любого ученика. Пока пользователь
        // сам не задал отображаемое имя, показываем нейтральную подпись.
        // ($email намеренно не используется — оставлен в сигнатуре для совместимости.)
        unset($email);
